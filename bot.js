...
const start = async function (err) {
  await sequelize.authenticate()
  await sequelize.sync({ force: true })
  if (err) return console.log('POSTGRES', err)
  // await agenda.start()
  ...
  bot.on('message', async (ctx) => {
    ...
    try {
      if (user.settings[0].command === 'feedback') {
        await setFeedback(ctx, id)
        await bot.telegram.sendMessage(
          ctx.chat.id,
          `Thank you! We read all the applications`
        )
      } else if (user.settings[0].command === 'search') {
        ctx.session.term = term
        console.log(ctx.session.term)
        if (user.settings[0].store === 'APP_STORE') {
        ...
        if (user.settings[0].store === 'GOOGLE_PLAY') {
      ...
      } else if (user.settings[0].command === 'setstore') {
        ...
        user.settings[0].command !== 'feedback' &&
        user.settings[0].command !== 'search'
      ) {
  ...
  bot.on('callback_query', async (ctx) => {
    try {
      ...
      if (user.settings[0].command === 'subscription') {
        if (data[1] === 'loadMore') {
            ...
            if (user.subscription[0].apps > 2)
              return ctx.reply(
                ctx.i18n.t('checkPlan', {
                  requiredNum: 2,
                  extraNum: user.subscriptions[0].apps - 2,
                })
              )
            break
          case 'MINI':
            if (user.subscriptions[0].apps > 6)
              return ctx.reply(
                ctx.i18n.t('checkPlan', {
                  requiredNum: 6,
                  extraNum: user.subscriptions[0].apps - 6,
                })
              )
            break
          default:
      ...
      if (data.state === 'pay') {
        ...
        switch (data.plan) {
          case 'BASIC':
            invoiceTitle = 'Basic'
            InvoiceDescription = 'Stores: AppStore & Google Play  Apps count: 2'
            amount = 4
            break
          case 'MINI':
            invoiceTitle = 'Mini-Studio'
            InvoiceDescription = 'Stores: AppStore & Google Play  Apps count: 6'
            amount = 7
            break
          default:
            invoiceTitle = 'Crew'
      ...
      /* REVIEWS (NEXT) */
      if (user.settings[0].command === 'lastReview') {
        const appId = data[0]
        const page = data[2]
        ...
        if (data[1] === 'next') {
          ...
          if (user.currentApps[0].store === 'AS') {
            ...
            await reviewsByCountryGS(
              ctx,
              appId,
              user.currentApps[0].nextPaginationToken,
              user.currentApps[0].country
            )
        }
      }
      ...
      /* COMMAND SEARCH */
      if (data[1] === 'add') {
        const reviews = await AppReviews.AppReviewsSchema.findOne({
          where: { appId: data[0], store: data[2] },
        })
        if (!reviews) {
          let reviews = await AppReviews.AppReviewsSchema.create({
            appId: data[0],
            store: data[2],
          })
      ...
      if (err.message === 'no subscription')
        ...
        await ctx.bot.telegram.sendMessage(ctx.chat.id, 'Limit is exceeded...')
