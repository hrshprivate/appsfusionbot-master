...
async function reviewsByCountryGS(ctx, appId, nextPaginationToken, country){
    try{
        ...
        await UserBot.updateOne({chatId: ctx.chat.id}, {
            $set: { 
                "currentApp.nextPaginationToken": newPaginationToken,
                "currentApp.country": country },
        },{new: true, runValidators: true})
        if(!apps.data[0]) return ctx.bot.telegram.sendMessage(ctx.chat.id, "There is no reviews by your query...") 
        for(let idx in apps.data){
            ...
            const opts = formOpts(idx, apps.data.length-1, appId, newPaginationToken)
            await ctx.bot.telegram.sendMessage(ctx.chat.id, ctx.i18n.t('reviewLast', {app:apps.data[idx], title, rating}), opts)
        }
    }catch(err){
        console.log(err)
...
async function lastReviewsGS(ctx, page, appId){
    const app = await AppReviews.findOne({appId, store: 'GS'})
   ...
   // console.log("START - ",start, "END - ", end)
    for(let idx = start; idx <= end; idx++){ 
        if(!reviews[idx] && idx == start ) ctx.bot.telegram.sendMessage(ctx.chat.id, "There is no reviews by your query...")
