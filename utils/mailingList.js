...
async function reviewsLoadMore(chatId, {appId, storeName}){
    try{
      //  console.log("reviews load More", appId, storeName)
        const subscription = await Subscription.findOne({chatId, appId: appId, store: storeName})
       
        const reviews = subscription.reviews
        const end = subscription.reviews.length < 3 ? subscription.reviews.length : 3
        ...
        if(reviews.length > end ){
            ...
            await bot.telegram.sendMessage(chatId,`You have ${reviews.length - end} more new reviews for ${app.title}` , opts)
        } 
        await Subscription.findOneAndUpdate(
            { chatId, appId: appId, store: storeName },
...
async function reviewsTimeTable(chatId, appId, storeName, user){
    try{
       ...
       //console.log()
        let appReviews = await AppReviews.findOne({ appId, store: storeName})
        ...
        if(!idxOfLastReview && reviews.length){
            ...
            await UserBot.findOneAndUpdate(
                { chatId: chatId },
                { $set: {"settings.command": "subscription"}} )
            return
        }
        if( idxOfLastReview == 0 || !reviewsForMessage.length ){
            ...
            for(let idx = start; idx < end; idx++){ 
                ...
                if(subscription.store === 'AS'){
                    ...
                    message = formMessageGS({app:reviewsForMailing[idx], title, rating})
                }
                if(idx == end) await bot.telegram.sendMessage(chatId, message, opts)
                else await bot.telegram.sendMessage(chatId, message, opts)
            }
            ...
            if(newReviews.length ){
                const opts = formOptsEnd(appId, storeName)
                await bot.telegram.sendMessage(chatId,`You have ${newReviews.length} more new reviews for ${app.title}` , opts)
            } 
            await Subscription.findOneAndUpdate(
                { chatId, appId: appId, store: storeName },
...
async function markAllReviewsRead(chatId, {appId, storeName}){
    try{
      //  console.log("mark all reviews read")
        await Subscription.findOneAndUpdate(
            {chatId, appId: appId, store: storeName},
            {reviews: [], end: 0},
            {new: true})
