...
async function searchInsideBothApps(term, page, id){
    const savedApps = await Application.findOne({chatId: id})
    ...
    for (let idx = start; idx <= end; idx++) {
        ...
        if(idx % 2 == 0 && appStoreApps[idx]){
            ...
            const opts = formOptsDoubleSearch(appIdVerified, 'AS', key, page, idx, end, description, savedAppsNames)
            c.push({
            ...
            const opts = formOptsDoubleSearch(appIdVerified, 'GS', key, page, idx, end, description, savedAppsNames)
            c.push({
...
async function   printGoogleApps(apps, term, page, ctx, counterStart, counterEnd, id){
    const savedApps = await Application.findOne({chatId: id})
    const savedAppsNames = savedApps.applications.map(savedApp => {
        return {
            appId: savedApp.appId,
    ...
    for(let app = counterStart; app < counterEnd; app++ ){
        ...
        if(counter === 3){
            ...
            await ctx.bot.telegram.sendPhoto(ctx.chat.id, apps[app].icon ,optsLastElem)
        }else{
...
async function nextListOfGoogleApps(ctx, data, user, key){
    try{
      ...
      //  console.log("All apps google ", response.length)
        ...
        await printGoogleApps(apps, key, data.page, ctx, counterStart, counterEnd, ctx.chat.id)
        }catch(err){
...
function formOptsDoubleSearch(appId, store, key, page, currIdx, lastIndex, description, savedApps){
    let isSavedApp = false
    savedApps.forEach(app=>{
